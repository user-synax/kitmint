...
import { Copy, Check, Share2, Eye, Calendar, Globe, Lock, RefreshCcw, Sparkles, ChevronLeft } from 'lucide-react';
...
const CopyIcon = ({ text, label }) => {
  ...
  return (
    <span onClick={handleCopy} className="cursor-pointer p-1 rounded hover:bg-surface-2 transition-colors touch-target-mobile flex items-center justify-center">
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-8 pb-16 animate-fade-in">
      ...
      <div className="mb-6 lg:hidden">
        <Button variant="ghost" asChild className="pl-0 text-text-secondary hover:text-text-primary">
          <Link href="/dashboard">
        ...
        <div className="mb-8 bg-surface-2 border border-border rounded-xl p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 group">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-text-secondary leading-relaxed">Upgrade to Pro for unlimited kits and block-level refreshing.</p>
            </div>
          </div>
          <Button asChild size="sm" className="w-full sm:w-auto bg-primary hover:bg-primary-hover text-white font-bold h-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-10 lg:gap-12">
        ...
        <div className="space-y-12">
          ...
          <section>
            <p className="text-xs font-bold text-[#4ADE80] uppercase tracking-widest mb-3">Generated Kit</p>
            <div className="border-l-2 border-[#16A34A] pl-4">
              <p className="text-[#9CA3AF] text-sm italic leading-relaxed">"{kit.ideaPrompt}"</p>
            </div>
          </section>
              ...
              {kit.brandNames.map((bn, i) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <h3 className="font-bold text-lg text-text-primary truncate pr-2">{bn.name}</h3>
                    {selectedNameIndex === i && <Badge className="bg-primary text-white text-[10px] px-2 h-5">Selected</Badge>}
                  </div>
                  <p className="text-xs text-text-secondary mt-2 line-clamp-2 leading-relaxed italic opacity-80">{bn.reason}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className={cn("transition-opacity", refreshingBlocks.fonts && "opacity-50")}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-surface border border-border rounded-xl p-6 space-y-3">
                <p className="text-[10px] text-text-muted uppercase tracking-widest font-bold">Heading Font</p>
                <p className="text-2xl font-bold text-text-primary" style={{ fontFamily: kit.fonts.heading }}>
                  {kit.fonts.heading}
                </p>
                <p className="text-sm text-text-secondary italic">Perfect for bold statements and titles</p>
              </div>
              <div className="bg-surface border border-border rounded-xl p-6 space-y-3">
                <p className="text-[10px] text-text-muted uppercase tracking-widest font-bold">Body Font</p>
                <p className="text-lg text-text-primary leading-relaxed" style={{ fontFamily: kit.fonts.body }}>
                  {kit.fonts.body}
                </p>
                <p className="text-sm text-text-secondary italic">Designed for readability and clarity</p>
              </div>
            </div>
          </section>
          ...
          <section className={cn("transition-opacity", refreshingBlocks.landingCopy && "opacity-50")}>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="bg-surface border border-border rounded-xl p-5 relative group">
                  <div className="absolute top-4 right-4">
                    <CopyIcon text={item.value} label={item.label} />
                  </div>
                  <p className="text-[10px] text-text-muted uppercase mb-2 font-bold">{item.label}</p>
                  <p className="text-text-primary pr-8 leading-relaxed">{item.value}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className={cn("transition-opacity", refreshingBlocks.productHunt && "opacity-50")}>
            <SectionLabel blockKey="productHunt">Product Launch</SectionLabel>
            <div className="grid grid-cols-1 gap-4">
              <div className="bg-surface border border-border rounded-xl p-5 relative group">
                ...
                <p className="text-[10px] text-text-muted uppercase mb-2 font-bold">Launch Tagline</p>
              ...
              <div className="bg-surface border border-border rounded-xl p-5 relative group">
                ...
                <p className="text-[10px] text-text-muted uppercase mb-2 font-bold">Launch Description</p>
          ...
          <section className={cn("transition-opacity", refreshingBlocks.pricingCopy && "opacity-50")}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[kit?.pricingCopy?.tier1, kit?.pricingCopy?.tier2, kit?.pricingCopy?.tier3].map((tier, i) => (
                <div key={i} className="bg-surface border border-border rounded-xl p-5 relative flex flex-col justify-center group min-h-[120px]">
                  <div className="absolute top-4 right-4">
                    <CopyIcon text={tier} label={`Tier ${i+1}`} />
                  </div>
                  <p className="text-[10px] text-text-muted uppercase mb-3 font-bold">Tier {i+1}</p>
                  <p className="text-text-primary text-sm pr-6 leading-relaxed">{tier}</p>
                </div>
              ))}
            </div>
          </section>
        ...
        <div className="relative">
          <div className="lg:sticky lg:top-20 space-y-4">
            <div className="bg-[#111111] border border-[#1F2937] rounded-xl p-6 space-y-6 overflow-hidden relative">
              ...
              <div className="space-y-4 relative z-10">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-mono text-text-primary font-bold">{kit.views}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="flex items-center text-text-secondary">
              ...
              <div className="space-y-4 relative z-10">
                {kit.isPublic ? (
                  <div className="space-y-3">
                    ...
                    <Button variant="outline" onClick={handleShare} className="w-full border-border hover:bg-surface-2 text-text-primary h-10 gap-2 font-semibold touch-target-mobile">
                  ...
                  <div className="space-y-3">
                    {isOwner ? (
                      <>
                        <Button className="w-full bg-primary hover:bg-primary-hover text-white font-bold h-11 touch-target-mobile" onClick={handlePublish} disabled={isPublishing}>
              ...
              {!session ? (
                <div className="space-y-3 relative z-10">
                  <Button variant="ghost" asChild className="w-full border-border hover:bg-surface-2 text-text-primary h-10 font-semibold touch-target-mobile">
                ...
                <div className="text-center py-2 relative z-10">
                  {isOwner ? (
                    ...
                    <Button variant="ghost" className="w-full border-border hover:bg-surface-2 text-text-primary h-10 font-semibold touch-target-mobile">
                      Save to my kits
                    </Button>
                  )}
                </div>
            ...
            <Button variant="link" asChild className="w-full text-text-muted hover:text-text-primary text-xs font-medium">
